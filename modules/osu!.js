    ...
    commands: {
        ...
        track: {
            ...
            func: async function(msg, args, data) {
                ...
                if (!tracking[msg.guild.id][msg.channel.id]) tracking[msg.guild.id][msg.channel.id] = {};
                switch (cmd) {
                    case "add":
                        let user = await osuapi.user.get(
                            name,
                        ...
                        if (user) {
                            ...
                            msg.channel.createMessage("debug.." + user.user_id, userScores[0].beatmap_id)
                        } else {
                            msg.channel.createMessage("A user named " + name + " does not exist.");
                        }
                        break;
                    case "remove":
